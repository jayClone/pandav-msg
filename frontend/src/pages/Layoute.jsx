...
export default function Layoute({ initialTab = "chats" }) {
  ...
  const [allGroups, setAllGroups] = useState([]); // Add groups state
  ...
  useEffect(() => {
    ...
    const handleUserOffline = ({ toUserId }) => {
      setOnlineUsers((prevUsers) => {
        ...
        return updatedUsers.length !== prevUsers.length
          ? updatedUsers
          : prevUsers;
      });
  ...
  const filteredUsers = useMemo(() => {
    // Show groups if groups tab is active
    if (activeTab === "groups") {
      return allGroups
        .filter((group) =>
        ...
        .sort((a, b) => {
          const aIsPinned = pinnedChats.includes(a.groupId || a.id);
          const bIsPinned = pinnedChats.includes(b.groupId || b.id);
    ...
    // Show chats for chats tab - always use allUsers (both online and offline)
    ...
    return usersToFilter
      ...
      .sort((a, b) => {
        ...
        // Pinned chats first, then online users, then offline
  ...
  return (
    ...
    >
      ...
      {/* MIDDLE SIDEBAR - Chat/Group List */}
      <div
      ...
      >
        ...
        {/* Theme Settings */}
        {showThemeSettings && (
          <div className="p-4 bg-[rgb(var(--bg-secondary))]/50 border-b border-[rgb(var(--border-secondary))] space-y-3 animate-in slide-in-from-top">
            <div className="space-y-2">
              <p className="text-xs font-semibold text-gray-400 uppercase">
                Background
              </p>
              <div className="grid grid-cols-3 gap-2">
                {Object.keys(bgImages).map((bg) => (
                  <button
                    key={bg}
                    onClick={() => setBgImage(bg)}
                    className={`px-2 py-1 rounded text-xs font-medium transition-all ${bgImage === bg ? "ring-2 ring-green-500" : "ring-1 ring-gray-600"} bg-gray-700 text-gray-300 capitalize hover:ring-green-400`}
                  >
                    {bg}
                  </button>
                ))}
        ...
        {showSettings && !showThemeSettings && (
          <div className="p-4 bg-[rgb(var(--bg-secondary))]/50 border-b border-[rgb(var(--border-secondary))] space-y-2 animate-in slide-in-from-top">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-400 flex items-center gap-2">
                {soundEnabled ? (
                  <Volume2 className="w-4 h-4" />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((item) => {
              ...
              const id =
                activeTab === "groups" ? item.groupId || item.id : item.userId;
              const name = item.name;
              const isPinned = pinnedChats.includes(id);
              const unreadCount = unreadCounts[id] || 0;
              const isOnline = activeTab === "chats" ? item.online : false;
        ...
        {selectedUserId ? (
          <>
            ...
            <div className="p-3 sm:p-4 bg-[rgb(var(--bg-secondary))] sm:glass-effect border-b border-[rgb(var(--border-secondary))] flex items-center justify-between gap-2 sm:gap-3">
              ...
              <button className="p-2 hover:bg-[rgb(var(--bg-hover))] rounded-lg transition-all text-gray-400 hover:text-green-400 flex-shrink-0">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <p className="wrap-break-word  leading-relaxed" style={{
                                    color: bgImages[bgImage].textColor,
                          }}>
                            {m.message}
                          </p>
            ...
            <div className="p-3 sm:p-4 bg-[rgb(var(--bg-secondary))] sm:glass-effect border-t border-[rgb(var(--border-secondary))]">
              ...
              <div className="flex items-end gap-2 sm:gap-3">
                ...
                <button
                  ...
                  className={`p-2 sm:p-3 rounded-xl transition-all shadow-lg flex-shrink-0 ${
                    messageInput.trim()
